...
void main() {
  ...
  test('Nghiệp vụ: Chuyển đổi lỗi kỹ thuật Firebase thành thông báo tiếng Việt',
      () async {
    ...
    result.fold(
      onSuccess: (_) =>
          fail('Lỗi logic: Đáng lẽ phải trả về Failure khi mất mạng!'),
      onFailure: (error) {
        ...
        expect(
            error.message,
            isNot(contains(
                'Exception'))); // Không được hiện lỗi kỹ thuật thuần túy
      },
    );

    debugPrint(
        'Xác nhận: Hệ thống đã chuyển đổi lỗi Firebase thành thông báo thân thiện thành công.');
  });
}
